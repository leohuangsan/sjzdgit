...
{
    public class Arrow : GraphicsInterface
    {
        public UInt64 m_ArrowID;//箭头对象ID
        public Graphics m_Graphics;//画布句柄
        public Point m_StartPoint;//画线的起点
        public Point m_EndPoint;//画线的终点       
        public Pen m_Pen = new Pen(Color.Red, 1);//非箭头画笔
        public Pen redArrowPen = new Pen(Color.Red, 1);//箭头画笔
        ...
        public Arrow(UInt64 ArrowID, Point StartPoint, Point EndPoint)
        {
            this.m_ArrowID = ArrowID;
            this.m_StartPoint = StartPoint;
            this.m_EndPoint = EndPoint;
            //初始化箭头pen
            redArrowPen.CustomEndCap = this.m_ShareLineCap;
            redArrowPen.CustomStartCap = this.m_ShareLineCap;
            redArrowPen.DashStyle = System.Drawing.Drawing2D.DashStyle.Solid;//实线 
            redArrowPen.Color = Color.Red;//红色画笔         
        }
        public void DrawingGraphics()
        {
            m_Pen.Color = Color.Red;
            m_Pen.DashStyle = DashStyle.Solid;
            redArrowPen.Color = Color.Red;
            m_Graphics.DrawLine(redArrowPen, m_StartPoint, m_EndPoint);
            ...
            if (m_IsDrawDashFrame)
            {
                ...
                redArrowPen.Color = Color.White;
                m_Graphics.DrawLine(redArrowPen, m_StartPoint, m_EndPoint);//再画一次白线带箭头
               //画与当前线段垂直的两条线段
                m_Pen.Width = 1;
                m_Graphics.DrawLine(m_Pen, _VeticalSegmentPoints1[0], _VeticalSegmentPoints1[1]);
                m_Graphics.DrawLine(m_Pen, _VeticalSegmentPoints2[0], _VeticalSegmentPoints2[1]);
                ...
                m_Pen.DashStyle = DashStyle.DashDotDot;
                //画范围框
                m_Pen.Color = Color.White;
                m_Graphics.DrawPolygon(m_Pen, _NewVeticalSegmentPoints);
        ...
        public bool IsContained(int x, int y)
        {
            ...
            {
                if ((x >= m_StartPoint.X) && (x <= m_EndPoint.X))
                {
                    if (h < 10) return true;
                }
                else if (x < m_StartPoint.X)
                {
                    delta1 = Math.Abs(x - m_StartPoint.X);
                    if (h < 10 && (delta1 < 10)) return true;
                }
                else if (x > m_EndPoint.X)
                {
                    delta2 = Math.Abs(x - m_EndPoint.X);
                    if (h < 10 && (delta2 < 10)) return true;
                }
            }
            else//终点坐标不大于起点坐标
            {
                if ((x >= m_EndPoint.X) && (x <= m_StartPoint.X))
                {
                    if (h < 10) return true;
                }
                else if (x < m_EndPoint.X)
